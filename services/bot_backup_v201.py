# bot.py - Version 20.1 (Finale stabile Version mit kompletter Fehlerbehandlung)
import numpy as np
import pandas as pd
...
class GridBot:
    def __init__(self, 
        ...
        # Initialize with validated price
        if initial_price is None:
            raise ValueError("Initial price cannot be None. Provide valid price or ensure DataFrame has 'close' column")
        ...
        try:
            ...
            raise ValueError(f"Grid initialization failed: {str(e)}")
    ...
    def _calculate_grid_lines(self, lower: float, upper: float, num: int, mode: str) -> List[float]:
        """Calculate grid lines with validation"""
    ...
    def _calculate_base_coin_amount(self, total_investment: float, num_grids: int, initial_price: float) -> float:
        """Calculate consistent coin amount per grid with validation"""
        if initial_price <= 0:
        ...
        return round(grid_investment / initial_price, 8)

    def _initialize_grids(self, total_investment: float, initial_price: float):
        """Initialize grids with proper error handling"""
        try:
            # 1. Initial coin purchase (50% of capital)
            ...
            for price in self.grid_lines:
                ...
                # Neutral grid at exact initial price is optional
                
        except Exception as e:
            raise RuntimeError(f"Failed to initialize grids: {str(e)}")

    def process_candle(self, candle: pd.Series):
        """Process candle data with error handling"""
        try:
            if 'close' not in candle or 'open' not in candle:
                raise ValueError("Candle missing required fields")
            ...
            raise RuntimeError(f"Candle processing failed: {str(e)}")

    def _execute_trade(self, grid: GridState, candle: pd.Series):
        """Execute trade with FIFO accounting"""
        try:
            ...
            if grid.side == 'sell':
                ...
                # Return unsold coins to inventory
            ...
            raise RuntimeError(f"Trade execution failed at {grid.price}: {str(e)}")
...
def simulate_grid_bot(df: pd.DataFrame,
    ...
    """
    Main simulation function with complete error handling
