# bot.py – Version 8.1 (Profit Calculation Fixed)
import numpy as np
import pandas as pd
...
def simulate_grid_bot(df, total_investment, lower_price, upper_price, num_grids,
                      grid_mode, fee_rate):
    """Main grid bot simulation function with profit calculation fix"""
    if df.empty:
        return None
    ...
    total_trading_capital = trading_capital
    ...
    # Initialize position
    position = {
        "usdt": total_trading_capital * usdt_portion,
    ...
    # Per-grid values
    usdt_per_buy_grid = position["usdt"] / n_below if n_below > 0 else 0
    ...
    # CRITICAL FIX 1: Add initial buy prices to FIFO queue
    # For each SELL grid above initial price, add initial price as cost basis
    for _ in range(n_above):
        buy_queue.append(initial_price)
    
    # Execute initial BUYs at start price
    for grid_price in sorted(grid_lines):
        if grid_price < initial_price:
            # Execute initial BUY at grid prices below starting price
            if position["usdt"] >= usdt_per_buy_grid:
                fee = usdt_per_buy_grid * fee_rate
                amount_net = usdt_per_buy_grid - fee
                ...
                # CRITICAL FIX 2: Add buy price to FIFO queue
    ...
    for idx in range(1, len(df)):
        ...
        # Skip micro-fluctuations (0.15% of initial price)
        ...
        for grid_price in grid_sequence:
            ...
            # SELL execution
            if (state["side"] == "SELL" and 
                prev_price < grid_price <= current_price and
                ...
                if buy_queue:
                    ...
                    # print(f"SELL@{grid_price:.2f} - BUY@{buy_price:.2f} = Profit: {profit:.4f}")
            ...
            # BUY execution
                ...
                fees_paid += fee
                buy_queue.append(grid_price)  # Track for FIFO profit calculation
                ...
                trade_log.append({
                    ...
                    "amount": net_coin,
                    "fee": fee,
                    "profit": 0.0  # Profit calculated on sell
                })
                state["active"] = False
    ...
    return {
        ...
        "bot_version": "bot.py v8.1 (Profit Calculation Fixed)"
    }